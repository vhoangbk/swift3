...
- (void)jsq_configureMessagesViewController
{
    ...
    self.inputToolbar.contentView.textView.placeHolder = [StringUtilities getLocalizedString:@"enter_message" comment:@""];
...
- (void)viewDidLoad
{
    ...
    UITapGestureRecognizer *tapAvatar = [[UITapGestureRecognizer alloc] initWithTarget:self  action:@selector(handlerTabAvatar)];
...
- (void)finishSendingMessageAnimated:(BOOL)animated {
    ...
    if (self.automaticallyScrollsToMostRecentMessage && self.isBottom && !self.isScrolling) {
...
- (void)finishReceivingMessageAnimated:(BOOL)animated {
    ...
    NSLog(@"isBottom %zd", self.isBottom);
    NSLog(@"isScrolling %zd", self.isScrolling);

    if (self.automaticallyScrollsToMostRecentMessage && ![self jsq_isMenuVisible] && self.isBottom && !self.isScrolling) {
        [self scrollToBottomAnimated:animated];
    }
...
- (UICollectionViewCell *)collectionView:(JSQMessagesCollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!isMediaMessage) {
        ...
        //2 : loading
        //3 : fail
        cell.btnRetryUploadImage.hidden = YES;
        if ([messageItem status] == 1){
            
        }else if ([messageItem status] == 2){
            UIView *loadding = [[UIView alloc] initWithFrame:CGRectMake(0, 0, cell.mediaView.bounds.size.width, cell.bounds.size.height)];
            loadding.backgroundColor = [UIColor grayColor];
            loadding.alpha = 0.5;
            
            UIActivityIndicatorView *spinner = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
            
            [spinner setCenter:CGPointMake((cell.bounds.size.height * 1.5 - 10)/2, cell.bounds.size.height/2)];
            
            [cell.mediaView addSubview:loadding];
            [cell.mediaView addSubview:spinner];
            [spinner startAnimating];
        }else if ([messageItem status] == 3){
            UILabel *lbError = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, cell.mediaView.bounds.size.width, cell.bounds.size.height)];
            lbError.textAlignment = NSTextAlignmentCenter;
            lbError.textColor = [UIColor redColor];
            lbError.text = [StringUtilities getLocalizedString:@"image_upload_fail" comment:@""];
            lbError.numberOfLines = 0;
            [cell.mediaView addSubview:lbError];
            
            cell.btnRetryUploadImage.hidden = NO;
        }
    }
...
//                            cell.avatarImageView.highlightedImage = [avatarImageDataSource avatarHighlightedImage];
                            ...
                            [cell.avatarImageView sd_setImageWithURL:[NSURL URLWithString:[avatarImageDataSource avatarImageLink]] placeholderImage:[UIImage imageNamed:@"avatar_no_image"]];
//                        }
    ...
    if (self.showViewLike == YES) {
        cell.viewLike.hidden = NO;
        
        if ([messageItem isLike] == YES) {
            [cell.imgHeart setImage:[UIImage imageNamed:@"btn_heart_like"]];
            cell.lbLikeNumber.textColor = [UIColor redColor];
        }else{
            [cell.imgHeart setImage:[UIImage imageNamed:@"btn_heart_dislike"]];
            cell.lbLikeNumber.textColor = [UIColor grayColor];
        }
        
        if ([messageItem numberLike] == 0) {
            cell.lbLikeNumber.hidden = YES;
        }else{
            cell.lbLikeNumber.hidden = NO;
            cell.lbLikeNumber.text = [NSString stringWithFormat:@"%zd", [messageItem numberLike]];
        }
        
    }else{
        cell.viewLike.hidden = YES;
    }
    
    cell.imgFlagItem.image = [messageItem imageFlagItem];
                                 ...
                                 atIndexPath:(NSIndexPath *)indexPath
{
    if (self.showTypingIndicator && [kind isEqualToString:UICollectionElementKindSectionFooter]) {
        collectionView.userTyping = self.userTyping;
...
- (void)collectionView:(JSQMessagesCollectionView *)collectionView
        didTapLikeView:(UIView *)likeView
           atIndexPath:(NSIndexPath *)indexPath{}
...
- (IBAction)pressExpandOrCollapse:(id)sender {
	[self pressExpandOrCollapse];
}

- (void)pressExpandOrCollapse{
