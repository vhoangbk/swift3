...
- (void)applyLayoutAttributes:(UICollectionViewLayoutAttributes *)layoutAttributes
{
    ...
    if ([self isKindOfClass:[JSQMessagesCollectionViewCellIncoming class]]) {
        ...
        self.avatarViewSize = CGSizeMake(40, 40);
    }
...
- (void)jsq_handleTapGesture:(UITapGestureRecognizer *)tap
{
    ...
    }else if(CGRectContainsPoint(self.viewLike.frame, touchPt)){
        [self.delegate messagesCollectionViewCellDidTapLike:self];
    }
    else {
...
- (IBAction)didPressRetryButton:(UIButton *)accessoryButton
{
    [self.delegate messagesCollectionViewCellDidPressRetry:self];
}

- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange{
    if ([URL.absoluteString hasPrefix:@"http"]) {
        [self.delegate messagesCollectionViewCellDidTapLink:URL];
        return NO;
    }
    return YES;
}
