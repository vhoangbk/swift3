...
- (JSQMessagesTypingIndicatorFooterView *)dequeueTypingIndicatorFooterViewForIndexPath:(NSIndexPath *)indexPath
{
    ...
    footerView.lbUserTyping.text = [NSString stringWithFormat:@"%@%@", self.userTyping, [StringUtilities getLocalizedString:@"is_typing" comment:@""]];
...
- (void)messagesCollectionViewCellDidTapLike:(JSQMessagesCollectionViewCell *)cell{
    NSIndexPath *indexPath = [self indexPathForCell:cell];
    if (indexPath == nil) {
        return;
    }
    [self.delegate collectionView:self didTapLikeView:cell.viewLike atIndexPath:indexPath];
    
}

- (void)messagesCollectionViewCellDidPressRetry:(JSQMessagesCollectionViewCell *)cell{
    NSIndexPath *indexPath = [self indexPathForCell:cell];
    if (indexPath == nil) {
        return;
    }
    
    [self.delegate collectionView:self didPressRetryButton:cell.btnRetryUploadImage atIndexPath:indexPath];
}

- (void)messagesCollectionViewCellDidTapLink:(NSURL *)nsUrl{
	[self.delegate collectionView:self didTapLink:nsUrl];
}
